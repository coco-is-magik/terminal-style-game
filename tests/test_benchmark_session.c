...
static void test_policy(void **state) {
    ...
    assert_false(benchmark_session_is_active(RUN_MODE_BENCHMARK_LIGHTING));
    ...
    assert_int_equal(benchmark_session_total_scenario_frames(0), BENCHMARK_WARMUP_FRAMES);
    ...
    assert_string_equal(benchmark_session_result_name(BENCHMARK_RESULT_NOT_APPLICABLE), "fail");
    ...
    assert_int_equal(benchmark_session_exit_code(BENCHMARK_RESULT_FAIL_ALLOCATION), 1);
}
