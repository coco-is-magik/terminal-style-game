...
static void app_ui_resources_destroy(AppUiResources *ui) {
    ...
    ui_canvas_destroy(ui->menu_exit);
...
static bool app_ui_resources_create(AppUiResources *ui, int grid_width,
    ...
    if (!ui->staging || !ui->menu || !ui->menu_exit || !ui->hud || !ui->editor || !ui->footer ||
...
int app_main(int argc, char* argv[]) {
    ...
    if (!ren) {
        ...
        if (mode == RUN_MODE_UI_THEME_DEMO || mode == RUN_MODE_UI_MOTION_DEMO ||
            mode == RUN_MODE_UI_WORKBENCH || mode == RUN_MODE_UI_EDITOR)
        ...
        return display_acceptance_mode || mode == RUN_MODE_UI_THEME_DEMO ||
            mode == RUN_MODE_UI_MOTION_DEMO || mode == RUN_MODE_UI_WORKBENCH ||
            mode == RUN_MODE_UI_EDITOR ? 1 : 0;
    }
    ...
    if (mode == RUN_MODE_UI_THEME_DEMO || mode == RUN_MODE_UI_MOTION_DEMO ||
        ...
        if (mode == RUN_MODE_UI_THEME_DEMO) {
        ...
        } else if (mode == RUN_MODE_UI_WORKBENCH) {
            ...
            UiEditorRuntimeResult result = ui_editor_runtime_run(
                ren, grid, cfg->target_fps, "assets");
            result_code = (int)result;
            diagnostic_ok = result == UI_EDITOR_RUNTIME_OK;
            diagnostic_out_of_memory = false;
        }
    ...
    double animation_event_start_ms = 0.0;
    ...
    while (!input.quit) {
            ...
            if (input.confirm && count > 0) {
                ...
                if (active_layout) {
                    ...
                    if (action) {
                        ...
                        animation_event_menu = active_menu;
                        animation_event = UI_ANIMATION_EVENT_ACTIVATE;
        ...
        ui_canvas_clear(ui_resources.menu_exit);
